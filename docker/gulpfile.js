 ...
 * scss to css - js to js - images to img - icons to icons - fonts to fonts
 *
 * `themesrc/themes/custom/project/js`
 *  is piped to
 ...
 * ex: `libraries/node/example.full.scss`
 *
 * === Preset tasks ===
...
function buildSass() {
  const tasks = config.map(function(entry) {
    return src(['themes/custom/' + entry.themeName + '/**/*.scss', '!themes/custom/' + entry.themeName + '/**/example.*.scss'])
...
function buildJavascript() {
  const tasks = config.map(function(entry) {
    return src(['themes/custom/' + entry.themeName + '/**/*.js', '!themes/custom/themes/custom/' + entry.themeName + '/**/example.*.js'])
      .pipe(sourcemaps.init())
...
function buildImages() {
  const tasks = config.map(function(entry) {
    return src('themes/custom/' + entry.themeName + '/images/**/*')
      .pipe(changed('../docroot/themes/custom/' + entry.themeName + '/' + customDest + '/img'))
      .pipe(imagemin({progressive: true}))
      .pipe(dest('../docroot/themes/custom/' + entry.themeName + '/' + customDest + '/img'));
  });
...
function buildIcons() {
  ...
  const tasks = config.map(function(entry) {
    return src('themes/custom/' + entry.themeName + '/icons/**/*.svg')
      .pipe(iconfontCss({
        fontName: 'themeIcons',
        path: 'themes/custom/' + entry.themeName + '/vendor/_icons-template.scss',
        targetPath: '../../../../../../themesrc/themes/custom/' + entry.themeName + '/vendor/_icons.scss',
...
function testJsLint() {
  const tasks = config.map(function(entry) {
    return src(['themes/custom/' + entry.themeName + '/**/*.js', '!themes/custom/' + entry.themeName + '/**/example.*.js'])
      .pipe(eslint())
      .pipe(eslint.format())
...
function watchSass() {
  config.map(function(entry) {
    watch(['themes/custom/' + entry.themeName + '/**/*.scss', '!themes/custom/' + entry.themeName + '/**/example.*.scss'], series(buildSass, testSassLint));
  });
}

function watchJavascript() {
  config.map(function(entry) {
    watch(['themes/custom/' + entry.themeName + '/**/*.js', '!themes/custom/' + entry.themeName + '/**/example.*.js'], series(buildJavascript, testJsLint));
  });
}
